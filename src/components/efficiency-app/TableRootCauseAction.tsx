...
import { VariableCause, VariableCauseAction } from '@/lib/APIs/useGetVariableCause'
        ...
        return (
            <>
                ...
                {(node.children && node.children.length > 0) && node.children.map((child, index) => (
                    <TableRootCause key={child.id} node={child} level={level + 1} checkRoot={checkRoot} parentId={parentId} isAction={false} />
                ))
                }
                {(node.actions && node.actions.length > 0) && node.actions.map((child, index) => (
                    <TableRootCause key={child.id} node={child} level={level} checkRoot={checkRoot} parentId={parentId} isAction={true} />
                ))
                }
            </>
        )
