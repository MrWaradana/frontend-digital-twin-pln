...
export function useGetDataPerformance(
  ...
  return useApiFetch(
    `${EFFICIENCY_API_URL}/data?page=${page}&size=${size}&is_performance_test=${isPerformance}&search=${search}&parameter=${parameter}&status=${status}&group_id=${groupId}`,
    !!groupId || !!token,
    token,
    {
